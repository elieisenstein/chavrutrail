...
* **Design:** Strava‑inspired dark theme, clean, functional aesthetic
...
* Future contact via in‑app notifications or WhatsApp link only
...
* **Architecture:**
  * Supabase Database Webhooks → Edge Function → Expo Push API
...
#### Organizer Display & Trust Signals ⭐ NEW
...
#### Persistent User Preferences ⭐ NEW
...
* **Organizer Display & Statistics:**
  - Added foreign key: rides.owner_id → profiles.id
...
* Deep link strategy: External links → FeedStack, internal nav maintains context
* Profile visibility: Display name + stats only (no clickable profiles yet)
  - Testing demand before building full profile features

**Testing:**
* ✅ Deep links (cold + warm start)
* ✅ Real-time updates (2-device verification)
* ✅ Gender preference filtering
* ✅ Organizer display with accurate statistics
* ✅ Persistent filters across restarts
...
* Supabase webhooks → Edge Functions → Expo Push API
